...
class MyApp:
    ...
    def __init__(self, root):
        ...
        self.root._set_appearance_mode("system") #default
        ...
        # Left frame
        self.left_frame = ctt.CTkFrame(master=self.root,bg_color="#757780")
        
        # Template section frame
        self.template_section_frame = ctt.CTkFrame(master=self.left_frame, bg_color="#757780")
        
        # Markers frame
        self.markers_frame = ctt.CTkScrollableFrame(master=self.left_frame, bg_color="#757780")
        
        # Data Frame
        self.data_frame = ctt.CTkFrame(master=self.left_frame, bg_color="#757780")
        # Record control frame
        self.record_control_frame = ctt.CTkFrame(master=self.root, bg_color="#757780")
    ...
    def create_widgets(self):
        ...
        template_name_entry.grid(row=2, column=0)
        self.template_section_frame.grid(row=0, column=0, padx=5, pady=5, sticky="w")

        #markers section
        markers_label = ctt.CTkLabel(master=self.markers_frame, text="Markers")
        markers_entry = ctt.CTkEntry(master=self.markers_frame)
        markers_entry.bind("<Return>", lambda event: self.on_marker_entry(event))
        rotate_icon = ctt.CTkImage(light_image=Image.open(path.join(self.assets_path, "rotate_icon.png")), dark_image=Image.open(path.join(self.assets_path, "rotate_icon.png")), size=(20, 20))
        remove_all_markers_button = ctt.CTkButton(master=self.markers_frame,text="", command=self.remove_all_markers, image=rotate_icon, width=rotate_icon.cget("size")[0], height=rotate_icon.cget("size")[1], bg_color="transparent", fg_color="transparent")
        ...
        markers_label.grid(row=0, column=0)
        markers_entry.grid(row=1, column=0)
        remove_all_markers_button.grid(row=0, column=1)
        self.markers_frame.grid(row=1, column=0, padx=5, pady=(10,0), sticky="nse")
        ...
        data_import_button.grid(row=1, column=0)
        manual_data_button.grid(row=2, column=0)
        self.data_frame.grid(row=2, column=0, padx=5, pady=5, sticky="w")
        ...
        records_label = ctt.CTkLabel(master=self.record_control_frame, text="Records")
        delete_all_records_button = ctt.CTkButton(master=self.record_control_frame, text="Delete All Records")
        delete_selected_record_button = ctt.CTkButton(master=self.record_control_frame, text="Delete Selected Record")
        #records section
        records_listbox = tk.Listbox(master=self.record_control_frame)
        records_scrollbar = tk.Scrollbar(master=self.record_control_frame)
        records_listbox.config(yscrollcommand=records_scrollbar.set)

        records_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        records_listbox.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
    ...
    def remove_marker(self, marker):
        if marker in self.markers:
            ...
            for widget in self.markers_frame.grid_slaves():
